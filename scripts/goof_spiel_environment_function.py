...
def rollout_last_prompt_and_completion_parallelized_curriculum(
    ...
    def run_single_prompt(index: int, prompt: str):
        ...
        # --- Reset Environment (POST /reset) ---
        episode_seed = random.randint(0, 9999999)
        payload = {"task_id": game_id, "seed": episode_seed, "opponent": "mcts"}
...
def rollout_full_prompt_and_completion_parallelized_curriculum(
    ...
    # Strategy weight decays from 0.3 → 0.0 as curriculum reaches full game length,
    # so the model progressively shifts from imitating a heuristic to winning outright.
    _FINAL_MAX_TURN = 13
    DYNAMIC_STRATEGY_WEIGHT = max(0.0, 0.3 * (1.0 - (current_max_turn - 1) / _FINAL_MAX_TURN))
    ...
    def run_single_prompt(index: int, prompt: str):
        ...
        while not done and (turn_number < current_max_turn):
            ...
            # Each step is scored independently — no gate that silences future rewards.
            bid_card = extract_bid_from_action(action_to_send, formatted_observation)
            total_strategy_opportunities += 1
            if bid_card is not None and bid_card == prize_card:
        ...
        if not done:
            # Partial episode — focus entirely on per-step strategy adherence
            shaped_reward = immediate_rewards + strategy_ratio
        else:
            # Full episode — blend: strategy weight decays to 0 as curriculum matures,
            # so the model is ultimately rewarded only for winning the game.
            shaped_reward = (
                DYNAMIC_STRATEGY_WEIGHT * strategy_ratio +
                (1.0 - DYNAMIC_STRATEGY_WEIGHT) * train_reward +
                immediate_rewards
            )
        ...
        print("============")
        print(f"id: {game_id}, seed: {episode_seed}, max_turn: {current_max_turn}, hints: {use_hints}, "
              f"strategy_w: {DYNAMIC_STRATEGY_WEIGHT:.2f}, "
              f"Strategy: {strategy_followed_count}/{total_strategy_opportunities} ({strategy_ratio:.2%}), "
              f"game_score: {train_reward:.3f}, shaped: {shaped_reward:.3f}")
        print("============")
