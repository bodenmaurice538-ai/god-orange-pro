...
GRPO_CONFIG = {
    # Changes vs original:
    # - rollouts_per_stage: 1280 → 320 (4× faster curriculum, reach full game much sooner)
    # - initial_max_turn: 1 → 3 (skip the trivially easy single-turn warmup)
    # - num_generations: 4 → 8 (better advantage estimation; batch_size halved to compensate)
    # - beta: reduced for small models (0.01-0.02 → 0.005) to allow bolder policy updates
    # - lora_r/alpha: 32/64 → 64/128 (more capacity for complex game strategy)
    "0_1_b": {
        "lr": 3e-5,
        "distributed": "ddp",
        "gpu_count": 1,
